...
base_path="/home/uib/PLOME/plome_ws/src/stereo_plome/stereo_tests/test_calamardo/"
img="1700483929385930316_left_image.png"
...
if(results[0].masks is not None):
    ...
    save_img_path=os.path.join(out_path,masks_inf_folder,"my_test_mask.jpg")
    # cv2.imwrite(save_img_path,mask_final,[cv2.IMWRITE_JPEG_QUALITY, 0])
    Image.fromarray(mask_final).save(save_img_path)
    print("Writing mask to: ", save_img_path)
    print("LA MASCARA FINAL!! ",np.unique(mask_final))
    print("Writing mask to: ",save_img_path)

    read_mask = cv2.imread(save_img_path, cv2.IMREAD_GRAYSCALE)
    print("Read Mask Shape:", read_mask.shape)
    print("Read Mask Unique Values:", np.unique(read_mask))
