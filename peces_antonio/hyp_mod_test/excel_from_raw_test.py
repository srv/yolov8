...
raw_data_path = r"C:\Users\haddo\yolov8\peces_antonio\hyp_mod_test\raw_val_data.json"
save_path = r"C:\Users\haddo\yolov8\peces_antonio\hyp_mod_test\test_hyps_results.csv" # CSV extension pls
...
df = pd.DataFrame(columns=['model', 'F1(B)', 'F1(M)', 'mAP50(B)', 'mAP50(M)', 'mAP50-95(B)', 'mAP50-95(M)'])
...
for model_version in res.keys():
    ...
    for idx in range(1, 6): #N folds
        current = res[model_version][f"fold_{idx}"]
